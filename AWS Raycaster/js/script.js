...
let sprintRate = 2;// sprint is this number * regular speed
let speedDampening = 0.15; //how fast you slow down, 0 makes you go on ice, 1 is instant
let maxSpeed = 0.5;
let recoilSeverity = 3;
...
/** @type {PlayerPosition} */
let playerpos = { x: 20, y: 15, rotation: 270 };
...
fire1.src = "assets/fire1.png";
const fire2 = new Image();
fire2.src = "assets/fire2.png";
const fire3 = new Image();
fire3.src = "assets/fire3.png";
const fire4 = new Image();
fire4.src = "assets/fire4.png";
let currentAnimationFrameFire = fire1
//fire end
const signature = new Image();
signature.src = "assets/signature.png";
const brick = new Image();
brick.src = "assets/redbrick.png";
const bluestone = new Image();
bluestone.src = "assets/bluestone.png";
...
function UIHandler() {
    ...
    ctu.drawImage(crosshair, myUI.width / 2 - 10, myUI.height / 2 - 10, 20, 20);
...
//current map start
//makeline(startX,startY,endX,endY,material,bool if it is see through, just put "wall" for now if isnt a moving wall)
makeLine(0,0,0,500,     "material-rainbow",false,"wall")
makeLine(0,0,500,0,     "material-rainbow",false,"wall")
makeLine(500,500,500,0,   "material-rainbow",false,"wall")
makeLine(500,500,0,500,   "material-rainbow",false,"wall")
// boundary wall ^^^
makeLine(   10, 15, 18, 8,   "materialimagebluestone",false,"wall")
makeLine(   18, 8,  29, 8,   "materialimagebrick",false,"wall")
makeLine(   29, 8,  37, 13,  "materialimagebluestone",false,"wall")
makeLine(   37, 13, 41, 23,  "materialimagebrick",false,"wall")
makeLine(   41, 23, 38, 33,  "materialimagebluestone",false,"wall")
makeLine(   38, 33, 30, 39,  "materialimagebrick",false,"wall")
makeLine(   30, 39, 20, 40,  "materialimagebluestone",false,"wall")
makeLine(   20, 40, 11, 34,  "materialimagebrick",false,"wall")
makeLine(   11, 34, 8,  25,  "materialimagebluestone",false,"wall")
makeLine(   8,  25, 10, 15,  "materialimagebrick",false,"wall")
//circle
makeLine(   19,  23, 27, 25,  "materialimagechainlinkFence",true,"wall")
makeLine(   27,  25, 22, 31,  "materialimagechainlinkFence",true,"wall")
makeLine(   22,  31, 19, 23,  "materialimagechainlinkFence",true,"wall")
//chainlink triangle
makeLine(   21,  25, 25, 26,  "materialimageanimatedFire",true,"wall")
makeLine(   25,  26, 22, 29,  "materialimageanimatedFire",true,"wall")
makeLine(   22,  29, 21, 25,  "materialimageanimatedFire",true,"wall")
//fire triangle
makeLine(   21,  25, 22, 25,  "materialimagesignature",true,"moving")
//signature to animate 
//current map end
...
function reset() {
    ...
    currentFrame = 1;
    playerpos = { x: 20, y: 15, rotation: 270 };
}
...
function materialEncyclopedia(materialName, wallDistanceFromOrigin) {
    switch (materialName) {
        case "imagesignature":
            return new TextureMaterial(signature, parseFloat(getDecimalPart(wallDistanceFromOrigin)) * signature.width);
        case "imagebrick":
            return new TextureMaterial(brick, parseFloat(getDecimalPart(wallDistanceFromOrigin)) * brick.width);
        case "imagebluestone":
            return new TextureMaterial(bluestone, parseFloat(getDecimalPart(wallDistanceFromOrigin)) * bluestone.width);
        case "imagebarrel":
            return new TextureMaterial(barrel, parseFloat(getDecimalPart(wallDistanceFromOrigin)) * barrel.width);
        case "imageanimatedFire":
            return new TextureMaterial(currentAnimationFrameFire, parseFloat(getDecimalPart(wallDistanceFromOrigin)) * currentAnimationFrameFire.width);
        case "imagetestTexture1":
            return new TextureMaterial(testTexture1, parseFloat(getDecimalPart(wallDistanceFromOrigin)) * testTexture1.width);
        case "imagebluby":
            return new TextureMaterial(bluby, parseFloat(getDecimalPart(wallDistanceFromOrigin)) * bluby.width);
        case "imagechainlinkFence":
            return new TextureMaterial(chainlinkFence, parseFloat(getDecimalPart(wallDistanceFromOrigin)) * chainlinkFence.width);
        case "rainbow":
...
function testAnim() {
    let temp = 22  
    if (animcount !== 0) {
        const animationAdress = vectorMapData.findIndex(i => i.wallFunction == "moving");
        temp = vectorMapData[animationAdress].start.x
        if (animationAdress >= 0) {
            vectorMapData.splice(animationAdress, 1), mapData.splice(animationAdress, 1);
        }
    }
    const animationCountResult = tempSwitchboard(temp)
    makeLine(animationCountResult.startX,animationCountResult.startY,animationCountResult.endX,animationCountResult.endY, "materialimagesignature", true, "moving");
    animcount++;
    //23,  20, 22, 20
    /**
     * 
     ...
     * @returns 
     */
    function tempSwitchboard(input){
        switch (input) {
            case 22:
                return {startX:23,startY:25,endX:22,endY:25}
            case 21:
                return {startX:22,startY:27,endX:21,endY:27}
            default:
                return {startX:21,startY:25,endX:20,endY:25}
        }
        }
}
/**
...
let count = 0;
let mapToggleCooldown = false
let fireAnimationframe = 1
function gameClock() {

    if(animcount<5000&&currentFrame%10==0){testAnim()}
    moveMaker();
    movementExecuter();
    if (controller[9].pressed) {
        if (!mapToggleCooldown) {
            mapToggle = !mapToggle; mapToggleCooldown = true; setTimeout(() => {
    ...
    if (!gametickPause) {
        currentFrame++;
        if(currentFrame%4==0){switch (fireAnimationframe) {
            case 1:
                fireAnimationframe++
                currentAnimationFrameFire = fire1
                break;
                    case 2:
                fireAnimationframe++
                currentAnimationFrameFire = fire2
                break;            case 3:
                fireAnimationframe++
                currentAnimationFrameFire = fire3
                break;
            default:
                fireAnimationframe=1;
                currentAnimationFrameFire = fire4
...
setTimeout(() => {
    vectorMapData.forEach(element => {
        if(element.material=="materialimageanimatedFire"){vectorMapData.splice(vectorMapData.indexOf(element), 1)}
    });
}, 5000);
setTimeout(() => {
    vectorMapData.forEach(element => {
